...
/*problems with runaway in cities after deleting the pointer*/
#include<iostream>
#include<cstdio>
#include<cstring>
#include<iomanip>
...
public:
            ...
            if(weapon[(i+curWeapon)%weaponCnt]->isAvail()){
                ...
                switch(weaponID){
                    case 0:{//swords
                        ...
                        #ifdef DEBUG
                            cout<<this->group<<' '<<this->type<<' '<<this->id<<' ';
                            cout<<"used sword"<<endl;
                        #endif 
    ...
    void getHurt(int injury){
        lifePoint-=injury;
        #ifdef DEBUG
        cout<<group<<' '<<type<<' '<<id<<" lost "<<injury<<", now remaining "<<lifePoint<<endl;
        #endif
    }
    bool isDead(){
        return lifePoint<=0;
    ...
    void takeWeapon(Warrior & enemy){
        ...
        for(i = 0 ; i < enemy.weaponCnt && enemy.weapon[i]->id != 2;++i){
            ...
            #ifdef DEBUG
            cout<<this->group<<' '<<this->type<<' '<<this->id<<" took "<<i<<' ';
            cout<<enemy.weapon[i]->type<<" from "<<enemy.group<<' '<<enemy.type;
            cout<<' '<<enemy.id<<endl;
            #endif
        }
        for(int j = enemy.weaponCnt-1; j >= i && enemy.weapon[j]->id == 2;j--){
            ...
            #ifdef DEBUG
            cout<<this->group<<' '<<this->type<<' '<<this->id<<" took "<<j<<' ';
            cout<<enemy.weapon[j]->type<<" from "<<enemy.group<<' '<<enemy.type;
            cout<<' '<<enemy.id<<endl;
            #endif
        }
        countWeapon();
...
#ifdef DEBUG
    void getWeapon(int _id){
        switch(weaponList[_id%3][0]){
            case 's': {
                weapon[weaponCnt++] = new sword(this);
                weaponListCnt[0]++;
                break;
            }
            case 'b': {
                weapon[weaponCnt++] = new bomb(this);
                weaponListCnt[1]++;
                break;
            }
            case 'a': {
                weapon[weaponCnt++] = new arrow(this);
                weaponListCnt[2]++;
                break;
            }
            default: break;
...
public:
    ...
    /*
     city attributes and battle procedure   
    */
...
int main(){
    ...
    for(int caseCnt = 1 ; caseCnt <= casenum; caseCnt++){
        ...
        for(tt = 0; tt<=T ;tt+=5){
            ...
            #ifdef DEBUG
            //    for(int i = 0 ; i < redHQ.curWarListIdx;++i){
            //        if(redHQ.warList[i]!=NULL)
             //           cout<<redHQ.warList[i]->getType()<<endl;
              //  }
            #endif
            ...
            switch(min){
                ...
                case 10:{//march on
                    ...
                    for(int i = 0; i<=N+1; ++i){
                        ...
                        /*{
                        cout<<"---------------now checking city "<<i<<" and "<<i+1<<endl;
                        ...
                        if(i==N){
                            if(cityList[N+1].redWarrior!=NULL){// blue is taken down
                                cityList[i+1].redWarrior->marchOn(i+1);
                                cout<<setfill('0')<<setw(3)<<hour<<':'<<setfill('0')<<setw(2)<<min<<' ';
                                cout<<"red "<<cityList[N+1].redWarrior->getType()<<' '<<cityList[N+1].redWarrior->getID()<<' ';
                                cout<<"reached blue headquarter with "<<cityList[N+1].redWarrior->getElements()<<' ';
                                cout<<"elements and force "<<cityList[N+1].redWarrior->getForce()<<endl;
                            ...
                            if(cityList[i+1].redWarrior!=NULL){
                                cityList[i+1].redWarrior->marchOn(i+1);
                                cout<<setfill('0')<<setw(3)<<hour<<':'<<setfill('0')<<setw(2)<<min<<' ';
                                cout<<"red "<<cityList[i+1].redWarrior->getType()<<' '<<cityList[i+1].redWarrior->getID()<<' ';
                                cout<<"marched to city "<<cityList[i+1].idx<<" with "<<cityList[i+1].redWarrior->getElements()<<' ';
                                cout<<"elements and force "<<cityList[i+1].redWarrior->getForce()<<endl;
                            }
                        }
                        if(i==N){
                                cityList[i+1].blueWarrior==NULL;
                        }
                        cityList[i].blueWarrior = cityList[i+1].blueWarrior;
                        
                        if(i==0){
                            if(cityList[0].blueWarrior!=NULL){// blue is taken down
                                cityList[0].blueWarrior->marchOn(0);
                                cout<<setfill('0')<<setw(3)<<hour<<':'<<setfill('0')<<setw(2)<<min<<' ';
                                cout<<"blue "<<cityList[0].blueWarrior->getType()<<' '<<cityList[0].blueWarrior->getID()<<' ';
                                cout<<"reached red headquarter with "<<cityList[0].blueWarrior->getElements()<<' ';
                                cout<<"elements and force "<<cityList[0].blueWarrior->getForce()<<endl;
...
#ifdef DEBUG/*
    ninja test(100,100,80,"red");
    lion test2(100,100,70,"blue",100);
    cityList[15].redWarrior = &test;
    cityList[15].blueWarrior =&test2;
    cityList[15].idx = 15;
    test.getWeapon(0);
    test.getWeapon(0);
    test.getWeapon(1);
    ...
    Warrior * item = &test;
    for(int i =0 ; i <item->weaponCnt;++i){
        cout<<i<<": "<<item->weapon[i]->type<<' '<<item->weapon[i]->statusCode<<endl;
    }
    test2.getWeapon(1);
    test2.getWeapon(2);
    test2.getWeapon(2);
